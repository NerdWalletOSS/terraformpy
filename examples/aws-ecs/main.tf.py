...
Resource(
    'aws_iam_policy_attachment', 'spot_fleet',
    name='spot_fleet',
...
Resource(
    'aws_spot_fleet_request', 'mycluster_ecs_slaves',
    lifecycle=dict(
    ...
    launch_specification=[
        dict(
            ...
            subnet_id="${element(data.terraform_remote_state.vpc.private_subnet_ids, %d)}" % count,
            availability_zone="${element(data.terraform_remote_state.vpc.azs, %d)}" % count,
            key_name="nw-dev",
            vpc_security_group_ids=[sg.id],
            root_block_device=dict(
                volume_size="40",
                volume_type="gp2"
            )
        )
